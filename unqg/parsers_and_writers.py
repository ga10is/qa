...
def clozes2squadformat(clozes, out_fobj):
    assert all([c.question_text is not None for c in clozes]
               ), 'Translate these clozes firse, some dont have questions'
    data = {cloze.paragraph.paragraph_id: {
        'context': cloze.paragraph.text, 'qas': []} for cloze in clozes}
    for cloze in clozes:
